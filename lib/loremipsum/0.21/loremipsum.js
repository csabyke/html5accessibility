...
function loremIpsumSentence(numWords){
	...
	sentence += capitalize(getLoremWord()) + " ";
	sentence += loremIpsum(numWords-1);
	sentence += getLoremEnding();
	sentence += " ";
...
function loremIpsumSentence2(numWords){
	...
	if(chance(50)){
		...
		sentence += loremIpsum(part1);
		sentence += ", ";
	} else {
		sentence += " ";
	}
	sentence += loremIpsum(numWords - part1 - 1);
	sentence += getLoremEnding();
	sentence += " ";
...
function loremIpsumParagraph(numWords){
	...
	while(words > 0){
		if(words > 10){
			...
			paragraph += loremIpsumSentence2(w);
			words = words - w;
		} else {
			paragraph += loremIpsumSentence2(words);
			words = 0;
		}
	}
